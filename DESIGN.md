...
cmux
 └ workspace: cclaunch            ← 最初に人間が1回だけ作る
     └ cclaunch run  (foreground) ← socket 権限を持つ唯一のプロセス
         └ spawn: cmux new-workspace --command "cd <cwd> && claude '<prompt>'"
...
| | |
|---|---|
| `cclaunch run` | cmux 内で常駐。キューを監視し、行があれば起動する。ログを stdout に吐く |
| `cclaunch add [-C <dir>] "<prompt>"` | 1 行 append。`-C` 省略時は cwd |
...
- `cmux new-workspace` にワークスペース名を渡すフラグがあるか。無ければ `cmux rename-workspace` を後追いで叩く。
- prompt のクォート。`--command` に渡す文字列へ安全に埋め込む必要がある。
